...
- **`lg_thinq` needs a patched `thinqconnect`**: `home-assistant.nix` overrides `pkgs.home-assistant` with a `packageOverrides` patch to `thinqconnect`. Upstream 1.0.13 (the version `lg_thinq`'s manifest pins) generates its MQTT client CSR with `OpenSSL.crypto.X509Req`, which pyOpenSSL removed in 25.3.0 — nixpkgs ships 26.3.0. Unpatched, MQTT setup raises `AttributeError: module 'OpenSSL.crypto' has no attribute 'X509Req'`, `lg_thinq` converts that to `ConfigEntryNotReady`, and since it forwards all 11 platforms *before* connecting MQTT — and HA's retry path never unloads them — every retry emits eleven `ValueError: Config entry ... has already been setup!` lines. That storm is a *symptom*; always look for the underlying reason, which `lg_thinq` only logs at `debug`. Key generation intentionally still uses the merely-deprecated `crypto.PKey` so the private key stays PKCS#8, the encoding `awscrt` already accepts. Drop the override once nixpkgs' `thinqconnect` moves to `cryptography`.
